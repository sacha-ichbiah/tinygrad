...
ALWAYS_CONTIGUOUS: set[Ops] = {Ops.CONTIGUOUS, Ops.ASSIGN, Ops.COPY, Ops.BUFFER, Ops.BUFFER_VIEW,
                     ...
                     Ops.DEFINE_LOCAL, Ops.DEFINE_REG, Ops.LOAD, Ops.KERNEL, Ops.KERNEL_MULTI, Ops.ENCDEC}
...
def realize_assign(ctx:dict[UOp, None], a:UOp) -> None:
  ...
  if a.src[1].op not in (Ops.KERNEL, Ops.KERNEL_MULTI): ctx[a] = None
...
def add_third_op_to_assign_to_track_shape(ctx:IndexingContext, assign:UOp):
  if assign.src[1].op in (Ops.KERNEL, Ops.KERNEL_MULTI): return None
...
def run_rangeify(tsink:UOp, debug:bool=False) -> tuple[UOp, IndexingContext]:
  ...
  for x in reversed(tsink_toposort):
    ...
    if x.op in (Ops.KERNEL, Ops.KERNEL_MULTI): continue
