...
class LinearSymplecticSystem:
  ...
  def verify_linearity(self, q0: Tensor, p0: Tensor, probe_eps: float | None = None) -> bool:
    q_flat = q0.detach().numpy().reshape(-1).astype(np.float64)
    p_flat = p0.detach().numpy().reshape(-1).astype(np.float64)
    n = q_flat.size
    eps = self.fd_eps if probe_eps is None else probe_eps
    for _ in range(4):
      q = np.random.randn(n).astype(np.float64)
      p = np.random.randn(n).astype(np.float64)
      dHdq, dHdp = self._grad_np(q, p)
      qn = q + eps * q
      pn = p + eps * p
      dHdq_n, dHdp_n = self._grad_np(qn, pn)
      if not np.allclose(dHdq_n, dHdq * (1.0 + eps), rtol=self.linear_tol, atol=self.linear_tol):
        return False
      if not np.allclose(dHdp_n, dHdp * (1.0 + eps), rtol=self.linear_tol, atol=self.linear_tol):
        return False
    return True
  ...
  def set_dt(self, dt: float) -> None:
    if not self._decomposed:
      raise ValueError("system must be decomposed before setting dt")
    if self.dt == dt:
      return
    if self._K is None or self._M_inv is None:
      raise ValueError("system must be decomposed before setting dt")
    self.dt = dt
    self._M_step = _build_leapfrog_step_matrix(self._K, self._M_inv, dt)
    self._propagator_cache.clear()
